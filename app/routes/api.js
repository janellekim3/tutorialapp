...
module.exports = function(router) {
            ...
            user.save(function(err) {
                if (err) {
                    ...
                    var email = {
                        ...
                        text: 'Hello ' + user.name + ', Thank you for registering at localhost.com. Please click on the link below to complete your activation: http://localhost:8080/activate/' + user.temporarytoken,
        ...
        User.findOne({ username: req.body.username }).select('username password activate').exec(function(err, user) {
            ...
            if (!user) {
                ...
                if (!validPassword) {
                    ...
                    res.json({ success: false, message: 'Accountis already activated.' });
                } else {
            ...
            user.save(function(err) {
                if (err) {
                    ...
                    var email = {
                        ...
                        text: 'Hello ' + user.name + ', You recently requested a new account activation link. Please click on the link below to complete your activation: http://localhost:8080/activate/' + user.temporarytoken,
    ...
    router.get('/renewToken/:username', function(req, res) {
        console.log('sdfsd---------------------f' + req.params.username + '000000000000000000000000');
    ...
    router.put('/edit', function(req, res) {
        ...
        User.findOne({ username: req.decoded.username }, function(err, mainUser) {
            if (err) throw err;
            if (!mainUser) {
                res.json({ success: false, message: "no usr found" });
            } else {
                if (newName) {
                    if (mainUser.permission === 'admin' || mainUser.permission === 'moderator') {
                        User.findOne({ _id: editUser }, function(err, user) {
                            ...
                            if (!user) {
                                ...
                                if (newPermission === 'user') {
                                    if (user.permission === 'admin') {
                                        if (mainUser.permission !== 'admin') {
                                            res.json({ success: false, message: 'Insufficient Permissions. You must be ana admin to downgrade another admin' });
                                        } else {
